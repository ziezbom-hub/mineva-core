...
// Logika modular untuk menangani penyimpanan, pembacaan, dan sinkronisasi data
function useLocalStorage(key, initialValue) {
  ...
  // Fungsi untuk memperbarui state dan localStorage secara aman
  const setValue = useCallback((value) => {
    try {
      // Dukung functional update seperti useState biasa
  ...
  // Sinkronisasi data jika ada perubahan di tab browser lain
  useEffect(() => {
...
export default function App() {
  // --- STATE DENGAN PERSISTENCE (LocalStorage) ---
  // Kita menggunakan useLocalStorage untuk data yang ingin dipertahankan saat refresh
...
const {
  ...
  // --- STATE NON-PERSISTENT (Hanya untuk interaksi sementara) ---
  ...
  // States Modal (Hanya sementara, akan tertutup jika refresh - Sesuai UX Standar)
  ...
  // --- Logika CRUD (Menggunakan setter dari useLocalStorage) ---
  const handleAddUser = () => {
    ...
    setUsers([...users, newUser]); // Ini otomatis menyimpan ke LocalStorage
  ...
  // --- RENDER LOGIC (Tetap sesuai desain asli Anda) ---
  if (!isLoggedIn) {
    return (
  ...
  />
      ...
      {/* --- MODALS (Tetap Menggunakan state internal, aksi Simpan akan men-trigger LocalStorage) --- */}
      {isAddUserModalOpen && (
