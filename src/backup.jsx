    ...
    <div className="min-h-screen bg-[#F8F9FD] dark:bg-slate-950 flex font-sans text-slate-900 dark:text-slate-100 transition-colors duration-300">
      ...
      >
          ...
          <nav className="flex-1 space-y-1.5 custom-scrollbar overflow-y-auto overflow-x-hidden">
            {NAV_ITEMS.map((item) => (
              ...
              >
                ...
                {isSidebarCollapsed && (
                  <div className="fixed left-20 px-3 py-1.5 bg-slate-900 text-white text-xs rounded-lg opacity-0 group-hover:opacity-100 pointer-events-none transition-opacity z-[60] ml-2 border border-slate-700">
                    {item.id}
                  </div>
                )}
              </button>
        ...
        <button 
          ...
          className="absolute -right-3.5 top-20 w-7 h-7 bg-white dark:bg-slate-900 border dark:border-slate-700 rounded-full hidden lg:flex items-center justify-center text-slate-400 hover:text-violet-600 shadow-sm z-50 transition-transform hover:scale-110"
        >
      ...
      <div className={`flex-1 flex flex-col transition-all duration-300 ${isSidebarCollapsed ? 'lg:pl-20' : 'lg:pl-64'}`}>
        ...
        <main className="flex-1 p-6 lg:p-10 scroll-smooth">
          ...
          {/* Table View (Users / Orders) */}
          {(activeTab === 'Users' || activeTab === 'Orders') && (
            <div className="space-y-6 animate-in fade-in duration-500">
               <div className="flex justify-between items-center">
                <h1 className="text-2xl font-black dark:text-white">{activeTab === 'Users' ? 'Daftar Pengguna' : 'Daftar Pesanan'}</h1>
                <button className="flex items-center gap-2 px-4 py-2.5 bg-violet-600 text-white font-bold rounded-xl text-sm shadow-lg transition-all">
                      ...
                      {(activeTab === 'Users' ? users : orders).map((item, idx) => (
                        <tr key={idx} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/50 transition-colors">
                          ...
                          <td className="px-6 py-4 font-semibold text-sm">
                            {activeTab === 'Users' ? <Badge variant={item.role}>{item.role}</Badge> : `Rp ${item.total.toLocaleString()}`}
                          </td>
                          <td className="px-6 py-4">
                          ...
                          <td className="px-6 py-4 text-center">
                            <div className="flex justify-center gap-2">
                              {activeTab === 'Users' ? (
                                <>
                                  <button onClick={() => startEditingUser(item)} className="p-2 text-slate-400 hover:text-violet-600 rounded-lg transition-colors"><Edit2 size={16}/></button>
                                  <button onClick={() => deleteUser(item.id)} className="p-2 text-slate-400 hover:text-rose-600 rounded-lg transition-colors"><Trash2 size={16}/></button>
                                </>
                              ) : (
                                <>
                                  <button onClick={() => startEditingOrder(item)} className="p-2 text-slate-400 hover:text-violet-600 rounded-lg transition-colors"><Edit2 size={16}/></button>
                                  <button onClick={() => deleteOrder(item.id)} className="p-2 text-slate-400 hover:text-rose-600 rounded-lg transition-colors"><Trash2 size={16}/></button>
                                </>
                              )}
                            </div>
                          </td>
          ...
          <div className="bg-white dark:bg-slate-900 w-full max-w-md p-8 rounded-3xl shadow-2xl border dark:border-slate-800 animate-in zoom-in-95 duration-200">
            ...
            <div className="space-y-6">
              ...
              <div className="space-y-2">
                ...
                <select 
                  className="w-full px-4 py-3 bg-slate-50 dark:bg-slate-800 border border-slate-100 dark:border-slate-700 rounded-xl outline-none focus:ring-2 focus:ring-violet-500/50 transition-all dark:text-white"
